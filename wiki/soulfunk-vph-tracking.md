...
## ⚠️ 핵심 원칙 (2026-04-25 강화)

### 1. 실측 VPH 갱신 강제 (워크플로우 ⓪단계)

**위키 기록 VPH 그대로 인용 금지.** 매 Vol 기획 직전 yt-dlp로 즉석 재측정 의무.

- 같은 영상도 시간이 지나면 VPH가 변한다
- 바이럴 영상은 **오히려 상승**:
  - 04-12 `Late Night Bar` 영상 → D8 위키 293 → D12 346 → D13 371 → **D13 386 (계속 가속)**
- 실패 영상은 하강
- **매 Vol 기획 시 ⓪단계로 재측정 → 현재 살아있는 키워드로 결정**

### 2. 제목 3개 동시 생성 (A/B 테스트용 · 2026-04-25 신설)

**모든 Vol 제목 작업은 3개 안을 동시 생성한다.**

| # | 전략 | 목적 |
|:---:|---|---|
| 1 | **챔피언 영상 정확 매칭** | 최고 VPH 영상 추천 슬롯 직접 진입 |
| 2 | **인접 트래픽 풀 매칭** | 보조 풀 진입 (경쟁 분산) |
| 3 | **차별화 안** (control) | 비교 기준선, 포맷·각도 차이 |

**근거:**
- YouTube Studio "영상 단계 A/B 테스트" 기능 사용 (자동 노출 분배 + 14일 측정)
- 우리 vol.01 (Velvet Midnight Bourbon) 첫 적용 (2026-04-25)
- 14일 후 결과 → 다음 Vol 제목 공식에 반영 (학습 누적)
